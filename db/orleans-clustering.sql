-- ============================================================================
--  Orleans ADO.NET PostgreSQL 클러스터링 스키마 (Main + Clustering)
--  출처: dotnet/orleans v9.2.1
--    - Shared/PostgreSQL-Main.sql
--    - Orleans.Clustering.AdoNet/PostgreSQL-Clustering.sql
--    - Migrations/PostgreSQL-Clustering-3.7.0.sql (CleanupDefunctSiloEntriesKey 추가;
--      9.x 런타임 필수. 베이스 스크립트에는 아직 병합되지 않아 별도 부록으로 포함)
--  Orleans:ClusteringMode=adonet 모드에서 실로 멤버십 저장소로 사용.
--  db/ddl.sql(도메인 스키마)과 분리 관리. docker-entrypoint-initdb.d 02번으로 적재.
-- ============================================================================
...
-- ==== Clustering 3.7.0 migration: CleanupDefunctSiloEntriesKey ====
INSERT INTO OrleansQuery(QueryKey, QueryText)
VALUES
(
