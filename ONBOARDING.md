...
>
> **짝 문서**: 설계 근거·답변 스크립트는 [`docs/interview-prep.md`](docs/interview-prep.md),
> 시니어 예상 압박 질문+모범답안은 [`docs/interview-hotseat.md`](docs/interview-hotseat.md),
...
**이게 답하는 면접 질문**: [hotseat A3](docs/interview-hotseat.md#a3-인메모리-북이-투영이라며--commit-후반영-전-죽으면)(투영),
[interview-prep 3장](docs/interview-prep.md)(돈 보존).
...
| 3 | `Grains/OrderBookEngine.cs` `PlaceOrderAsync` — **검증** | 수량/단가 상한, **총액을 `Int128`로 계산**해 `MaxNotional` 초과 거부(오버플로=음수 에스크로=무한발행 차단). | 병뚜껑 무한발행 취약점을 원천 봉쇄한 자리(STAR ①). |
...
**이게 답하는 면접 질문**: [hotseat A1](docs/interview-hotseat.md)(DB 핫패스), [hotseat A2](docs/interview-hotseat.md)(에스크로 vs 정산),
[hotseat 5번/A5](docs/interview-hotseat.md)(고아 에스크로).
...
트랜잭션 순서:
0. **락 순서 선점** — `SELECT … FROM wallet WHERE player_id = ANY(@ids) ORDER BY player_id FOR UPDATE`.
   서로 다른 종목 grain의 동시 정산이 매수/매도 지갑을 **다른 순서로** 잠가 생기던 데드락(40P01)을
   `player_id` 오름차순 통일로 제거(STAR ②).
1. `trade` INSERT.
...
**이게 답하는 면접 질문**: [hotseat A2](docs/interview-hotseat.md), [interview-prep STAR ②](docs/interview-prep.md)(데드락),
[backend-audit 병뚜껑 무한발행](docs/backend-audit.md).

**스스로 답해보기**: (a) 낙관적 가드가 막는 시나리오를 하나 말해봐라. (b) 정수 절삭 수수료가 값을 새게
하지 않는 이유는? (c) 왜 지갑 행을 `ORDER BY player_id`로 잠그나?
...
**이게 답하는 면접 질문**: [hotseat A3·A5](docs/interview-hotseat.md), [interview-prep "서버가 죽으면?"](docs/interview-prep.md).
...
거래소를 살아있게 하는 경제 엔진 + 그리드. `MarketRepository.Raid.cs`, `MarketRepository.Stash.cs`, `Grains/StashGrain.cs`.
...
**이게 답하는 면접 질문**: [hotseat 1장(그레인/동시성)·A2](docs/interview-hotseat.md), [interview-prep 5장(게임 도메인)](docs/interview-prep.md).
...
**이게 답하는 면접 질문**: [hotseat A1(동시성)·9장(테스트 격리)](docs/interview-hotseat.md).
...
- [ ] **먼저 인정할 갭 7개**([hotseat 부록 A](docs/interview-hotseat.md))를 근거와 함께 말한다.
...
| 3:50–4:00 | 체크리스트 + [hotseat 5개 모범답안](docs/interview-hotseat.md) 소리 내어 | 즉답 리허설 |
