...
평평해지고, p50 지연이 그 큐 대기시간으로 상승한다(실측 180 ms).
...
**수정**: 정산 트랜잭션 시작에 이 트랜잭션이 건드릴 지갑 행을 **항상 playerId 오름차순으로 미리
잠갔다**(`MarketRepository.SettleFillAsync`). 매수자/판매자 역할과 무관하게 락 획득 순서가 전역적으로
일정해져 데드락 사이클이 성립하지 않는다. (당시엔 `WHERE player_id = ANY(ids) ORDER BY player_id
FOR UPDATE` 한 문장이었으나, Postgres 행 락은 정렬 노드 이전 스캔에서 획득되어 락 순서가 ordered
index scan 플랜일 때만 보장된다 → 이후 C#에서 정렬해 개별 `FOR UPDATE`로 잠그도록 하드닝, 플랜 비의존.)
...
동일하게 템플릿당 단일 호가창을 직접 매칭한다(**기존 동작 바이트 단위 보존** — 기존 회귀
스위트 전부 그대로 통과, 현재 총 125개).
...
스위치는 config-gated(`Redis:ConnectionString` 비면 인메모리 단일 인스턴스)라 기존 테스트/데모/
성능 경로는 불변이다.
