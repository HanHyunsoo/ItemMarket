...
> 대규모 리팩터(주머니·가변 스태시·다중 스택·레이드 at-risk 재설계·아이템 140종/스프라이트 다각화) 직후
...
- **강점(포트폴리오급):** "황무지 암시장 터미널" 무드가 확실히 산다(앰버/러스트, 모노 타입, `// MARKET`
  ...
  인형+주머니/리그/백팩(서버 검증, 140종 스프라이트)도 명확하고 테마 일관.
...
4. **첫 세션: 스프레드시트에 던져짐.** `/`가 140개 통조림으로 바로 리다이렉트. 캡·루프·첫 할 일 설명 없음.
...
- [x] **최종 QA fun#4 — High 위험 프리미엄 확대** 역전은 됐으나 프리미엄이 +19%뿐(Med와 사망률 54%↔55%
      ...
      드롭 가중치만 COMMON 15→11, EPIC 15→19로 이동(LEGENDARY 5% 유지). 시드 140종 직접 파싱 재검산:
...
- [x] **A-2** (후속 QA·Medium) 어드민 아이템 지급 원장 기록 — `AdminGrantStack/InstanceAsync`를
...
- [x] **#2** 마켓 카드 실시간 시세 — 배치 엔드포인트 `GET /api/market/tickers`(집계) + 카드에 최우선
      매수/매도 호가·최근 체결·유동성 점(활성 주문 수), 활동순 정렬, 15초 폴링. 호가 없는 종목은
      **벤더 참고가**(base_value±15% 스프레드, `vendorBid`/`vendorAsk` — 실거래 아님)로 "죽은 첫 화면"을
      해소. (`MarketRepository.GetTickersAsync`, `MarketView.vue`)
