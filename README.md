...
구현하고, 체결을 **단일 Postgres 트랜잭션으로 원자 정산**합니다. 나아가 **스태시/로드아웃 2-컨테이너
인벤토리** 위에서 **출격(at-risk 자산 잠금) → 탈출(귀속)/사망(소실)** 세션 상태머신을 원자적으로
정산하며, 고부하 동시성에서 **돈·아이템 보존을 테스트로 증명**합니다.

`C# / .NET 10` · `Orleans` · `PostgreSQL` · `SignalR` · `Redis` · `Vue 3` ·
**테스트 77개** · **부하 테스트로 데드락 발견 → p99 5.5× 개선**
...
- **도메인 상태머신 · 원자 정산** — 익스트랙션 레이드 세션(`RaidSessionGrain`, playerId 키): 스태시/
  로드아웃 2-컨테이너 인벤토리에서 **출격이 로드아웃을 at-risk로 잠그고**(매도 에스크로의 자산 잠금
  재사용), **탈출=반입+전리품 전량 귀속 / 사망=at-risk 전량 소실**을 각각 단일 Postgres 트랜잭션으로
  정산. 총량 보존·스태시(안전) 불가침을 불변식 테스트로 고정, 모든 이동은 append-only `item_ledger`로
  프로비넌스 회계.
...
- **품질 · 운영** — Testcontainers 기반 통합 테스트 우선(총 77개) · CI · Docker 한 방 실행 · Swagger ·
  어드민 GM 툴 · 풀스택(Vue 3).
...
```
스태시(안전, 10×12)  ──반입──▶  로드아웃(6×8)  ──출격──▶  ACTIVE(at-risk 잠금)
                                                            │
                                        탈출(생존) ──귀속──▶ 스태시로 자동 배치
                                        사망       ──소실──▶ at-risk 전량 소각 → 마켓 희소성
```

- **2-컨테이너 인벤토리**: `STASH`(안전 보관, 자동 배치) / `LOADOUT`(레이드 반출 칸). 반입/반출은
  `POST /api/stash/move`(`FromContainer`/`ToContainer`)로 **원자적** 처리. 총 소유량의 진실은
  인벤토리라 컨테이너 이동으로 늘거나 줄지 않음.
- **세션 상태머신**: `(없음) --start--> ACTIVE --extract--> EXTRACTED / --die--> DIED`. 플레이어당
  ACTIVE 세션 1개(부분 유니크 인덱스로 강제), 각 전이는 단일 Postgres 트랜잭션.
- **불변식**: 탈출은 반입+전리품 전량 귀속(총량 보존), 사망은 at-risk만 소실하고 **스태시(안전)는
  불가침**. 모든 이동은 append-only `item_ledger`(프로비넌스 회계)로 기록. 통합 테스트로 고정.
...
추가 화면: **장비**(`web/src/views/LoadoutView.vue` — 스태시/로드아웃 듀얼 그리드, 반입/반출) ·
**출격**(`web/src/views/RaidView.vue` — 준비/진행/결과 상태 화면).</sub>
...
```
      ...
      │  Market · Grid · 장비(Loadout) · 출격(Raid) · Wallet · Orders · Admin │
      └───────┬────────────────────────────────────────────┬────────────────┘
      ...
      │  StashGrain(스태시/로드아웃 2-컨테이너) · RaidSessionGrain(세션 상태머신)  │
...
dotnet test                                  # 77개: 단위 35 + 통합 38 + 밴딩 4
...
검증, 컨테이너 간 이동(반입/반출)·**레이드 세션 정산**(출격/탈출/사망 불변식·총량 보존), 가격밴드
격리. 순수 로직(기하·수수료)은 DB 없는 단위 테스트.
...
- **도메인 확장(구현 완료)**: **익스트랙션 세션 루프**(로드아웃 반출→생존 귀속/사망 소실→마켓 유입)와
  아이템 프로비넌스 원장(`item_ledger`), 2-컨테이너(스태시/로드아웃) 인벤토리는 **구현했습니다**.
- **도메인 확장(남은 부분)**: 보험, 실제 전투/루팅(현재 loot는 MVP 시뮬레이션 엔드포인트), 그리드
  회전·추가 컨테이너 타입(배낭/파우치 등), 이상거래(RMT) 탐지.
