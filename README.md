...
구현하고, 체결을 **단일 Postgres 트랜잭션으로 원자 정산**합니다. 나아가 **그리드 인벤토리(가변 스태시 +
내재 주머니) + 타르코프식 장비 슬롯**(헬멧·방어구·무기 + 중첩 리그·백팩 그리드) 위에서 **출격(at-risk
...
**테스트 117개** · **부하 테스트로 데드락 발견 → p99 5.5× 개선**
...
- **도메인 상태머신 · 원자 정산** — 익스트랙션 레이드 세션(`RaidSessionGrain`, playerId 키): 그리드
  인벤토리·장비 슬롯에서 **출격이 스태시 밖 전부(착용 장비+백팩·리그 내용물+주머니)를 at-risk로 잠그고**
  (매도 에스크로의 자산 잠금 재사용), **탈출=출격 시점 배치 그대로 제자리 복원 + 전리품 귀속 / 사망=at-risk 전량 소실**을 각각
...
- **품질 · 운영** — Testcontainers 기반 통합 테스트 우선(총 117개) · CI · Docker 한 방 실행 · Swagger ·
  어드민 GM 툴 · 풀스택(Vue 3).
...
```
스태시(안전, 12×가변)  ──착용/적재──▶  장비 슬롯 + 주머니 + 백팩·리그(중첩)  ──출격(존 선택)──▶  ACTIVE
   ▲  ▲                            헬멧·방어구·무기 + 1×4 주머니 + 중첩 그리드      │ (스태시 밖 전부 at-risk 잠금 + 제한시간)
   │  │                                        ▲ 루팅(서버 드롭테이블) ┐          │
   │  │                                        └───────────────────────┘ 사망확률↑ │
   │  └── 탈출(성공률 롤) ──▶ 출격 시점 배치 그대로 제자리 복원 + 전리품 귀속 ───────┤
   └───── 사망(확률/시간초과) ──▶ at-risk 전량 소각 → 마켓 희소성 ─────────────────┘
```

- **그리드 인벤토리 + 타르코프식 장비**: `STASH`(안전 보관, 가로 12 고정·세로 가변=`player.stash_rows`) /
  `POCKETS`(캐릭터 내재 주머니, 1×4) + **장비 슬롯 5종**(`Helmet`/`Armor`/`Weapon` 단일 슬롯,
  `Rig`/`Backpack`은 착용하면 **내부 그리드를 여는 중첩 컨테이너**). 스태시↔주머니↔중첩 그리드 이동은
  `POST /api/stash/move`(`FromContainer`/`ToContainer` + `ContainerInstanceId`), 슬롯 착용/해제는
  `POST /api/equipment/equip`·`unequip`로 **서버 권위·원자 처리**. 스택형은 한 칸당 `max_stack` 상한으로
  **여러 칸에 분할·병합**된다. 총 소유량의 진실은 인벤토리라 이동으로 늘거나 줄지 않음.
...
- **at-risk = 스태시 밖 전부**: 반입 대상은 착용 장비(헬멧/방어구/무기/백팩/리그) + 백팩·리그 **내용물** +
  주머니. **착용 장비만 있어도 출격 가능**(로드아웃 개념 없음). **스태시(안전)는 절대 at-risk가 아님**.
- **리스크/보상 루프**: 출격 시 **존(Low/Med/High)** 선택 — 존이 서버 **드롭테이블**의 rarity 가중치,
  loot당 **사망확률 상승률**, **기본 사망확률(floor)**, **출격 수수료**를 함께 결정한다(고위험=좋은
  드롭+빠른 사망확률+높은 진입 비용). 루팅은 클라이언트가 아이템·수량을 못 정하고 서버가 존 가중치로
  롤(무한 인플레 차단). 출격엔 **제한시간(deadline)**이 걸려 마감 초과 후 `extract`/`loot`는 탈출
  실패=사망으로 정산(lazy expiry). `extract`는 누적 사망확률로 **생환 롤**(floor에서 시작하므로
  0루팅 반입도 무위험이 아님) — "한 상자 더 vs 지금 탈출" 도박.
- **완결된 캡 경제**: 캡은 **출격 수수료**(recurring)와 **스태시 행 확장**(`POST /api/stash/upgrade`,
  점증 가격)으로 소각되고, 유한 드롭·거래로 유입돼 "파밍→거래→확장" 루프가 닫힌다. **리더보드**는
  지갑이 아닌 **순자산**(지갑+보유 아이템 기준가)으로 순위해 소비를 억제하지 않고, 마켓 카드는
  실시간 시세 + 호가 없는 종목엔 **벤더 참고가**(base_value 스프레드)로 항상 살아 있다.
- **불변식**: 탈출은 **반입 자산을 출격 전 자리 그대로 복원**하고 전리품을 귀속(총량 보존), 사망은
  at-risk만 소실하고 **스태시(안전)는 불가침**. 모든 이동은 append-only `item_ledger`(프로비넌스
  회계)로 기록. 통합 테스트로 고정.

### 인벤토리·컨테이너 데이터 모델 (DB 설계)

그리드 인벤토리 전체가 **테이블 하나(`stash_placement`)** 로 표현됩니다. 컨테이너 계층(가방 안 아이템,
리그 안 아이템)은 별도 테이블 없이 **자기참조 FK 하나**로 모델링합니다.

- **컨테이너 = 그냥 유니크 아이템**: 가방/리그는 특별 테이블이 아니라 `is_container=true`인 `item_instance`
  입니다(템플릿의 `container_w × container_h`가 내부 그리드 크기). 착용은 `player_equipment(player_id,
  slot, instance_id)`.
- **"어디에 있는가" = `(container, container_instance_id)` 쌍** — 배치 한 행이 놓인 *물리 컨테이너*:

  | `container` | `container_instance_id` | 위치 |
  |---|---|---|
  | `STASH` | `NULL` | 플레이어 스태시(안전) |
  | `POCKETS` | `NULL` | 내재 주머니 |
  | `CONTAINER` | 가방/리그 인스턴스 id | **그 가방/리그 안**(중첩 그리드) |

  → `container_instance_id`만으로는 스태시/주머니를 구분 못 함(둘 다 NULL). 반드시 `container`와 함께 읽음.
  `stash_placement`는 `item_instance`를 **두 번** 참조: `instance_id`(배치된 유니크 아이템 자신) +
  `container_instance_id`(그것을 담은 가방/리그). `CHECK ((container='CONTAINER') = (cinst IS NOT NULL))`로
  규칙 강제.
- **셀 유일성 · NULL 접기**: `uq_stash_cell(player_id, container, COALESCE(cinst, 센티넬UUID), x, y)` —
  물리 컨테이너의 한 칸엔 배치 최대 1개. NULL은 센티넬 UUID로 접어(Postgres에서 NULL은 유니크 비교 시
  서로 다르게 취급) 스태시/주머니에도 유일성이 걸리게 함. footprint(1×1 초과) 겹침은 그리드 기하 엔진이
  앱 레벨에서 강제.
- **다중 스택**: 같은 스택 템플릿이 여러 칸·여러 컨테이너에 나뉠 수 있음(각 스택 ≤ `max_stack`). 셀 유일성
  덕분에 스태시 (0,0)과 배낭 (0,0)이 안 부딪힘(물리 컨테이너가 다름).
- **at-risk 한 방 쿼리**: "가방을 착용하면 그 안 내용물도 통째로 at-risk"가 이 관계로 바로 나옴 —
  `WHERE container='CONTAINER' AND container_instance_id IN (착용한 백팩·리그 인스턴스들)` (+ 주머니 +
  장착 슬롯). 스태시는 조건에서 제외되므로 자연히 안전.
...
중첩 리그·백팩) + 주머니(1×4)를 한 화면에 통합, 드래그로 착용·이동) · **아이템 상세**
...
dotnet test                                  # 117개: 단위 41 + 통합 72 + 밴딩 4
...
컨테이너 간 이동(스태시↔주머니↔중첩)·**다중 스택 분할/병합**(max_stack)·**장비 착용/해제**(슬롯 매칭·중첩 컨테이너), **레이드 세션 정산**
...
- **도메인 확장(구현 완료)**: **익스트랙션 세션 루프**(반출→생존 시 제자리 복원/사망 소실→마켓 유입),
  아이템 프로비넌스 원장(`item_ledger`), 그리드 인벤토리(가변 스태시 + 내재 주머니 + 다중 스택) +
  **타르코프식 장비 슬롯**(헬멧·방어구·무기 + 중첩 리그·백팩 그리드), 아이템 상세 인스펙터, 레이드 이력 뷰,
  그리고 **완결된 게임 경제 루프** — 레이드 제한시간·loot당/기본 사망확률(리스크), 존별 서버 드롭테이블(유한 공급),
  캡 싱크(스태시 확장·출격 수수료), 마켓 실시간 시세(+벤더 참고가), 리더보드(최다 순자산·최다 생환)까지 **구현했습니다**.
- **도메인 확장(남은 부분)**: 보험, 실제 전투/루팅(현재 루팅은 서버 드롭테이블 시뮬레이션),
  그리드 회전, 아이템 내구도/수리, 이상거래(RMT) 탐지.
