...
아포칼립스/익스트랙션 슈터 세계관의 실시간 아이템 거래소입니다. 게임 서비스 백엔드로 만든 1인 포트폴리오이고,
중심은 주문서 매칭 엔진입니다. 동시 주문 경쟁을 Microsoft Orleans 단일 활성화로 락 코드 없이 직렬화하고,
체결은 에스크로와 단일 Postgres 트랜잭션으로 정산합니다. 돈·아이템이 보존되는지는 부하 후 SQL 불변식으로 확인합니다.

`C# / .NET 10` · `Orleans` · `PostgreSQL` · `SignalR` · `Redis` · `Vue 3` · **테스트 125개** ·
**부하 테스트로 데드락 발견 → p99 5.5× 개선**

<p align="center">
  <img src="docs/demo-trade.gif" alt="실시간 체결 데모 — 매도 호가를 넘는 매수 주문이 즉시 체결되고 호가창·체결·잔액이 SignalR로 리로드 없이 갱신" width="88%" />
  <br/>
  <sub><b>매도 호가를 넘는 매수 주문 → 즉시 체결.</b> 호가창·최근 체결·캡 잔액이 SignalR로 리로드 없이 갱신 (라이브 앱을 Playwright로 녹화).</sub>
</p>
...
> 시간이 없으면 이 섹션만 봐도 됩니다. 나머지(레이드 경제·그리드 인벤토리·풀스택·어드민)는 거래소를
> 뒷받침하는 부분이라 아래에 접어뒀습니다.

주문서 매칭 엔진을 Orleans(액터 모델)로 구현하면서 세 가지에 집중했습니다:

- **동시성 (락 코드 0줄)** — 종목당 grain이 단일 활성화 + 논-리엔트런트라 매칭이 단일 스레드처럼
  직렬화되고, 이중 체결·경쟁 상태가 생기지 않습니다. "재고 1개에 서로 다른 7명이 동시 매수 → 정확히 1건
  체결, dupe 0"을 통합 테스트로 고정했습니다.
- **정합성 (원자 정산)** — 주문 시점에 에스크로로 자산을 잠그고, 각 체결을 단일 Postgres 트랜잭션으로
  정산합니다. 부하 후 SQL 불변식으로 `발행 = 지갑+에스크로+소각`(diff 0)·아이템 보존·음수잔액 0을 확인합니다.
- **문제 해결 (실측)** — 직접 만든 부하 도구로 교차-grain 지갑 락 순서 데드락(40P01)을 발견하고, 락 정렬로
  p99를 973→175ms(5.5×) 줄였습니다. 데드락 0.
...
깊게 읽는 순서는 **[`ONBOARDING.md`](ONBOARDING.md) 2~3장**. 설계 결정을 스스로 반박·방어한
자문자답 노트는 [`docs/notes/`](docs/notes/)에 따로 뒀습니다(선택 읽기).
...
> 아래는 나머지 시스템·근거·자료입니다. 필요할 때 펼쳐 보세요.

<details>
<summary>📊 <b>데모 GIF</b> (그리드 DnD · 익스트랙션 레이드)</summary>

실시간 체결 GIF는 상단에 있습니다. 아래는 나머지 두 루프 — 라이브 앱을 Playwright로 녹화한 실동작 GIF입니다 (재현: [`tools/screenshots`](tools/screenshots)).

<table>
<tr>
<td width="50%"><b>그리드 드래그앤드롭(인벤토리)</b> — 아이템을 드래그해 재배치. 서버가 겹침·범위를 검증합니다(허용=초록 / 거부=빨강 + 원복).</td>
<td width="50%"><b>경제 엔진 — 익스트랙션 레이드 루프</b> — 존을 골라 <b>수수료</b>를 내고 출격, 루팅할수록 <b>사망확률이 오르고</b> 탈출 성공률이 떨어집니다("한 상자 더 vs 지금 탈출"). 여기서 나온 전리품이 거래소로 유입됩니다.</td>
</tr>
<tr>
<td><img src="docs/demo-grid.gif" alt="그리드 드래그앤드롭 데모" width="100%" /></td>
<td><img src="docs/demo-raid.gif" alt="익스트랙션 레이드 루프 데모" width="100%" /></td>
</tr>
</table>
...
<details>
<summary>🧩 <b>이 프로젝트가 다루는 것</b> (엔지니어링 주제 8가지)</summary>
...
> **2.2×의 기준선**: 밴드 샤딩 효과는 **동일 측정 조건(넓은 밴드)** 의 OFF 대비 649/**299** = 2.2×다.
> 표의 `hot 350`은 별도(좁은 밴드) 조건이라 649/350으로 계산하면 어긋난다 — 조건별 기준선과
> 상세는 [`docs/perf-report.md`](docs/perf-report.md) 참고.
...
<details>
<summary>🔁 <b>경제 엔진 — 익스트랙션 루프</b> (거래소의 수요·공급)</summary>

거래소에 실제 수요·공급이 돌도록, 아이템의 **공급원(faucet)**·**소각처(sink)**를 서버 권위·원자 정산으로
붙였습니다. 게임플레이 틱/전투는 범위 밖 — 이 백엔드는 게임 서버가 호출하는 서비스이며
...
- `docs/notes/` — **설계 자문자답 노트**(선택 읽기): 각 결정을 스스로 반박·방어한 Q&A.
  [`interview-prep.md`](docs/notes/interview-prep.md)(설계 근거·STAR), [`interview-hotseat.md`](docs/notes/interview-hotseat.md)(가장 아픈 압박 질문에 대한 자기검증)
