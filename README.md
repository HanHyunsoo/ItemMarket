...
### A. 원커맨드 풀스택 (Docker만 있으면 됨)

```bash
# postgres + redis + api + web 를 한 번에 빌드·기동 (프로필: app)
docker compose --profile app up --build

#   web  → http://localhost:8081   (nginx, SPA)
#   api  → http://localhost:8080   (/health, /api/...)
#   ddl(스키마+102종 아이템+시드)은 최초 1회 자동 적용
#   web 컨테이너는 시작 시 VITE_API_BASE 를 config.js 로 주입 → 이미지 재빌드 없이 API URL 교체 가능

docker compose --profile app down -v   # 스택 종료 + 볼륨 삭제
```

라이브 데모(:5080/:5173)와 겹치지 않게 대체 포트/이름으로 별도 스택을 띄우려면:

```bash
PG_CONTAINER=alt-db PG_PORT=5433 REDIS_CONTAINER=alt-redis REDIS_PORT=6380 \
API_CONTAINER=alt-api WEB_CONTAINER=alt-web API_PORT=8080 WEB_PORT=8081 \
docker compose -p alt --profile app up -d
```

### B. 로컬 개발 (DB만 Docker, api/web 은 호스트에서)

```bash
# 0) 사전 조건: .NET 10 SDK, Docker, Node 20+, jq

# 1) DB (스키마 + 102종 아이템 마스터 + 시드 자동 적용) — 프로필 없이 기본 up 은 postgres(+redis)만 띄운다
docker compose up -d
...
# 테스트 (전체 3개 프로젝트 — Integration/Banding 은 Testcontainers 로 일회용 Postgres 자동 기동)
dotnet test Solution1.sln    # UnitTests + IntegrationTests + BandingTests (CI 와 동일)
...
tests/                      # UnitTests + IntegrationTests + BandingTests (Testcontainers + WebApplicationFactory)
Dockerfile                  # API 이미지 (SDK 빌드→aspnet 런타임, Release publish)
web/Dockerfile              # 웹 이미지 (Vite 빌드→nginx, VITE_API_BASE 런타임 주입)
.github/workflows/ci.yml    # CI: 백엔드(3개 테스트) + 프론트(build+lint)
