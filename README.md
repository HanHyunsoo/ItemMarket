...
```
스태시(안전, 12×가변)  ──착용/적재──▶  장비 슬롯 + 주머니 + 백팩·리그(중첩)  ──출격──▶  ACTIVE
   ▲  ▲                            헬멧·방어구·무기 + 1×4 주머니 + 중첩 그리드      │ (스태시 밖 전부 at-risk 잠금)
   │  │                                                                          │
   │  └── 탈출(생존) ──▶ 출격 시점 배치 그대로 제자리 복원 + 전리품 귀속 ───────────┤
   └───── 사망       ──▶ at-risk(착용+주머니+중첩) 전량 소각 → 마켓 희소성 ────────┘
```
...
- **도메인 확장(구현 완료)**: **익스트랙션 세션 루프**(반출→생존 시 제자리 복원/사망 소실→마켓 유입),
  ...
  **타르코프식 장비 슬롯**(헬멧·방어구·무기 + 중첩 리그·백팩 그리드), 아이템 상세 인스펙터, 레이드 이력 뷰는 **구현했습니다**.
