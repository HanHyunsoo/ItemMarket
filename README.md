...
**테스트 112개** · **부하 테스트로 데드락 발견 → p99 5.5× 개선**
...
- **품질 · 운영** — Testcontainers 기반 통합 테스트 우선(총 112개) · CI · Docker 한 방 실행 · Swagger ·
  어드민 GM 툴 · 풀스택(Vue 3).
...
- **리스크/보상 루프**: 출격 시 **존(Low/Med/High)** 선택 — 존이 서버 **드롭테이블**의 rarity 가중치와
  loot당 **사망확률 상승률**을 함께 결정한다(고위험=좋은 드롭+빠른 사망확률). 루팅은 클라이언트가
  아이템·수량을 못 정하고 서버가 존 가중치로 롤(무한 인플레 차단). 출격엔 **제한시간(deadline)**이 걸려
  마감 초과 후 `extract`/`loot`는 탈출 실패=사망으로 정산(lazy expiry). `extract`는 누적 사망확률로
  **생환 롤** — "한 상자 더 vs 지금 탈출" 도박. 벌어들인 캡은 **스태시 행 확장**(`POST /api/stash/upgrade`,
  점증 가격)으로 소각돼 "파밍→거래→확장" 경제 루프가 닫힌다.
...
dotnet test                                  # 112개: 단위 41 + 통합 67 + 밴딩 4
...
- **도메인 확장(구현 완료)**: **익스트랙션 세션 루프**(반출→생존 시 제자리 복원/사망 소실→마켓 유입),
  ...
  **타르코프식 장비 슬롯**(헬멧·방어구·무기 + 중첩 리그·백팩 그리드), 아이템 상세 인스펙터, 레이드 이력 뷰,
  그리고 **완결된 게임 경제 루프** — 레이드 제한시간·loot당 사망확률(리스크), 존별 서버 드롭테이블(유한 공급),
  캡 싱크(스태시 확장), 마켓 실시간 시세, 리더보드(최다 캡·최다 생환)까지 **구현했습니다**.
